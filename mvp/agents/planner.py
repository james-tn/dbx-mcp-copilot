...
Data access rule:
- rely on the signed-in user's secure enterprise data access
- local-only testing helpers may allow territory overrides, but authenticated sessions should not
- if the seller asks how access works, explain that the planner uses their signed-in access through secure enterprise tools
"""
