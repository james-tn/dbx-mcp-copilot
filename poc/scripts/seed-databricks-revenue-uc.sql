-- Revenue Intelligence POC seed script (UC mode)
-- Time range: Apr 2023 through Mar 2026 (36 monthly periods).

CREATE SCHEMA IF NOT EXISTS __CATALOG__.ri_poc;

CREATE OR REPLACE TABLE __CATALOG__.ri_poc.dim_region (
  region_id STRING,
  region_code STRING,
  region_name STRING,
  country STRING
...
CREATE OR REPLACE TABLE __CATALOG__.ri_poc.dim_product (
  product_id STRING,
  product_family STRING,
  product_name STRING
);

CREATE OR REPLACE TABLE __CATALOG__.ri_poc.dim_sales_channel (
  channel_id STRING,
  channel_name STRING
);

CREATE OR REPLACE TABLE __CATALOG__.ri_poc.dim_customer_segment (
  segment_id STRING,
  segment_name STRING
);

CREATE OR REPLACE TABLE __CATALOG__.ri_poc.dim_date (
  date_key DATE,
  month_index INT,
  fiscal_year INT,
...
CREATE OR REPLACE TABLE __CATALOG__.ri_poc.fact_revenue (
  date_key DATE,
  region_id STRING,
  product_id STRING,
  segment_id STRING,
...
CREATE OR REPLACE TABLE __CATALOG__.ri_poc.fact_quota (
  date_key DATE,
  region_id STRING,
  product_id STRING,
...
CREATE OR REPLACE TABLE __CATALOG__.ri_poc.metric_catalog (
  metric_code STRING,
  metric_name STRING,
...
CREATE OR REPLACE TABLE __CATALOG__.ri_poc.principal_region_access (
  principal STRING,
  region_code STRING
);

INSERT OVERWRITE __CATALOG__.ri_poc.dim_region VALUES
...
INSERT OVERWRITE __CATALOG__.ri_poc.dim_product VALUES
...
INSERT OVERWRITE __CATALOG__.ri_poc.dim_sales_channel VALUES
  ('C1', 'Direct'),
  ('C2', 'Partner'),
  ('C3', 'Online');

INSERT OVERWRITE __CATALOG__.ri_poc.dim_customer_segment VALUES
  ('S1', 'Enterprise'),
  ('S2', 'Mid-Market'),
  ('S3', 'SMB');

INSERT OVERWRITE __CATALOG__.ri_poc.dim_date
SELECT
  d AS date_key,
...
INSERT OVERWRITE __CATALOG__.ri_poc.metric_catalog VALUES
...
INSERT OVERWRITE __CATALOG__.ri_poc.fact_revenue
WITH base AS (
  SELECT
    d.date_key,
    d.month_index,
  ...
  FROM __CATALOG__.ri_poc.dim_date d
  CROSS JOIN __CATALOG__.ri_poc.dim_region r
  CROSS JOIN __CATALOG__.ri_poc.dim_product p
  CROSS JOIN __CATALOG__.ri_poc.dim_customer_segment s
  CROSS JOIN __CATALOG__.ri_poc.dim_sales_channel c
),
calc AS (
  SELECT
    date_key,
...
INSERT OVERWRITE __CATALOG__.ri_poc.fact_quota
SELECT
  f.date_key,
  f.region_id,
  f.product_id,
  CAST(round(SUM(f.net_amount) * 1.06, 2) AS DECIMAL(18,2)) AS quota_amount
FROM __CATALOG__.ri_poc.fact_revenue f
GROUP BY f.date_key, f.region_id, f.product_id;

INSERT OVERWRITE __CATALOG__.ri_poc.principal_region_access VALUES
  ('grp_revenue_na', 'NA'),
...
CREATE OR REPLACE VIEW __CATALOG__.ri_poc.v_fact_revenue_secure AS
SELECT f.*
FROM __CATALOG__.ri_poc.fact_revenue f
JOIN __CATALOG__.ri_poc.dim_region r ON f.region_id = r.region_id
JOIN __CATALOG__.ri_poc.principal_region_access a ON a.region_code = r.region_code
